            ...
            <div class="card card-info">
              <div class="card-header">
                <h3 class="card-title">Update Post</h3>
                 <router-link to="/posts" class="btn btn-sm btn-info float-right">Back</router-link>
              </div>
              <!-- /.card-header -->
              <!-- form start -->
      ...
      loadThumbnail: function(e){
       ...
       updatePost: function(){
          ...
          this.form.post('/update-post')
                .then((response)=>{
                      //Show alert using Toastr
                    toastr.success('Post updated successfully!',{timeout: 3000});
                    get_this.$router.push("/posts");
      ...
      getPost: function(){
          ...
          axios.get("/edit-post/"+ this.$route.params.slug).then((response)=>{
              //response.data.category--- ekhane category namta hocce categoryController theke response ermaddhome pathano nam.So same name hoite hbe
                get_this.form.fill(response.data.post);//using this command set the data of input field
          })
      }
  }
 }
